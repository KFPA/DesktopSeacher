...
CDirBasicInfoMap::CDirBasicInfoMap(int _N){
    m_dwMaxCount=_N;
    m_pIndex=(PDirFrnIndexBlockNode*)g_pMemoryMgr->malloc(m_dwMaxCount*sizeof(PDirFrnIndexBlockNode));
    m_dwBlockCount=0;
	m_pLastNode = m_pIndex[m_dwBlockCount++] = (PDirFrnIndexBlockNode)g_pMemoryMgr->GetMemory(sizeof(DirFrnIndexBlockNode));
    m_pLastNode->Init();
...
void CDirBasicInfoMap::insert(DWORD BasicInfo,IndexElemType DirPtr)
{
    ...
    if(i==m_dwBlockCount){//当前插入的是最大FRN
        ...
        //pNode指向插入节点
        //寻找插入位置先
		size_t low = pNode->CurrentBegin, high = pNode->CurrentEnd - 1;
        int mid;         
        while(low<=high)
        {
            mid=low+((high-low)>>1);
...
void CDirBasicInfoMap::AddLastBlock()
{
    ...
    if(m_dwBlockCount==m_dwMaxCount){//索引BUF已满，增加之
		...
		m_pIndex = (PDirFrnIndexBlockNode*)g_pMemoryMgr->realloc(m_pIndex, m_dwMaxCount*sizeof(PDirFrnIndexBlockNode));
    }
}
void CDirBasicInfoMap::InsertBlock(int i)//在第i处插入新块
{
    for(int j=m_dwBlockCount++;j>i;--j) m_pIndex[j]=m_pIndex[j-1];
	m_pIndex[i] = (PDirFrnIndexBlockNode)g_pMemoryMgr->GetMemory(sizeof(DirFrnIndexBlockNode));
    m_pIndex[i]->Init();
    if(m_dwBlockCount==m_dwMaxCount){//索引BUF已满，增加之
        m_dwMaxCount+=INDEX_COUNT_DELT;
		m_pIndex = (PDirFrnIndexBlockNode*)g_pMemoryMgr->realloc(m_pIndex, m_dwMaxCount*sizeof(PDirFrnIndexBlockNode));
