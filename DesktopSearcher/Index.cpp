...
CIndex::CIndex(int _N){
    m_dwMaxCount=_N;
    m_pIndex=(PINDEX_BLOCK_NODE*)g_pMemoryMgr->malloc(m_dwMaxCount*sizeof(PINDEX_BLOCK_NODE));
    m_dwBlockCount=0;
	m_pLastNode = m_pIndex[m_dwBlockCount++] = (PINDEX_BLOCK_NODE)g_pMemoryMgr->GetMemory(sizeof(INDEX_BLOCK_NODE));
    m_pLastNode->Init();
    m_size=0;
...
CIndex::~CIndex(){
    ...
    for(size_t i=0;i<m_dwBlockCount;++i){
		g_pMemoryMgr->FreeMemory((PBYTE)m_pIndex[i]);
    }
	g_pMemoryMgr->free(m_pIndex);
}
void CIndex::Lock(){
...
void CIndex::AddLastBlock()
{
    ...
    if(m_dwBlockCount==m_dwMaxCount){//索引BUF已满，增加之
        ...
        m_pIndex=(PINDEX_BLOCK_NODE*)g_pMemoryMgr->realloc(m_pIndex,m_dwMaxCount*sizeof(PINDEX_BLOCK_NODE));                
    }
}
void CIndex::InsertBlock(int i)//在第i处插入新块
{
    for(int j=m_dwBlockCount++;j>i;--j) m_pIndex[j]=m_pIndex[j-1];
	m_pIndex[i] = (PINDEX_BLOCK_NODE)g_pMemoryMgr->GetMemory(sizeof(INDEX_BLOCK_NODE));
    m_pIndex[i]->Init();
    if(m_dwBlockCount==m_dwMaxCount){//索引BUF已满，增加之
        m_dwMaxCount+=INDEX_COUNT_DELT;
		m_pIndex = (PINDEX_BLOCK_NODE*)g_pMemoryMgr->realloc(m_pIndex, m_dwMaxCount*sizeof(PINDEX_BLOCK_NODE));
...
{
	size_t nIndex = i;
	assert(nIndex >= 0 && nIndex<m_dwBlockCount);
	PINDEX_BLOCK_NODE pNode = m_pIndex[nIndex];
    //[i ... m_dwBlockCount-1]
    --m_dwBlockCount;
	for (; nIndex<m_dwBlockCount; ++nIndex) m_pIndex[nIndex] = m_pIndex[nIndex + 1];
	g_pMemoryMgr->FreeMemory((PBYTE)pNode);
}
